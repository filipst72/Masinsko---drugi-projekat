...
class NN:
    def __init__(self, input_size=108, hidden_size1=128, hidden_size2 = 64, output_size=7, device='cpu'):
        self.device = device
        
        self.W1 = th.randn(input_size, hidden_size1, device=device) * np.sqrt(2.0 / input_size)
        self.W2 = th.randn(hidden_size1, hidden_size2, device=device) * np.sqrt(2.0 / hidden_size1)
        self.W3 = th.randn(hidden_size2, output_size, device=device) * np.sqrt(2.0 / hidden_size2)
        self.b1 = th.zeros(hidden_size1, device=device)
        self.b2 = th.zeros(hidden_size2, device=device)
        self.b3 = th.zeros(output_size, device=device)
    ...
    def backward(self, X, y_true, y_pred):
        ...
        dz3 = y_pred - y_true  
        self.dW3 = (self.a2.T @ dz3) / batch_size
        self.db3 = dz3.sum(dim=0) / batch_size

        dz2 = (dz3 @ self.W3.T) * self.relu_izvod(self.z2)
...
model = NN(input_size=108, hidden_size1=128, hidden_size2 = 64, output_size=7)
